...
test("encode produces 'height:tx_index'", () => {
  expect(cursor.encode(123, 5)).toBe("123:5");
  expect(cursor.encode(0, 0)).toBe("0:0");
});

test("encode returns null for invalid inputs", () => {
  expect(cursor.encode(-1, 0)).toBe(null);
  expect(cursor.encode(0, -1)).toBe(null);
  expect(cursor.encode("a", 0)).toBe(null);
  expect(cursor.encode(0, NaN)).toBe(null);
  expect(cursor.encode(0, Infinity)).toBe(null);
});

test("encode floors floats", () => {
  expect(cursor.encode(123.7, 5.2)).toBe("123:5");
});

test("decode null/empty yields the start position", () => {
  expect(cursor.decode(null)).toEqual({ height: 0, tx_index: 0 });
  expect(cursor.decode(undefined)).toEqual({ height: 0, tx_index: 0 });
  expect(cursor.decode("")).toEqual({ height: 0, tx_index: 0 });
});

test("decode parses valid cursors", () => {
  expect(cursor.decode("75880:3")).toEqual({ height: 75880, tx_index: 3 });
  expect(cursor.decode("0:0")).toEqual({ height: 0, tx_index: 0 });
});

test("decode rejects malformed cursors", () => {
  expect(cursor.decode("foo")).toBe(null);
  expect(cursor.decode("123")).toBe(null);
  expect(cursor.decode("-1:0")).toBe(null);
  expect(cursor.decode("1:2:3")).toBe(null);
  expect(cursor.decode(123)).toBe(null);
});

test("encode/decode roundtrip", () => {
  const original = { height: 75880, tx_index: 3 };
  const encoded = cursor.encode(original.height, original.tx_index);
